#!/bin/bash
# G-S3-HOLD (P8-safety.md S:8, P8.5): fly a Goto, inject OBSTACLE_TOO_CLOSE
# directly on /world/obstacle_map_local (perception:=false is sim.launch.py's
# own default, so this probe is the ONLY publisher -- proven below with
# `ros2 topic info` BEFORE the probe starts, R27 dieu 1), and confirm HOLD +
# ClearFault + resumed commanding via src/uav_safety/test/obstacle_hold_gate.py.
# Usage: bash scripts/verify_obstacle_hold.sh
...
PROBE=$WORKSPACE/src/uav_safety/test/obstacle_hold_gate.py
[ -f "$PROBE" ] || { echo "FATAL: $PROBE missing"; exit 1; }

# Same teardown discipline as verify_safety.sh (G-S2): a leftover process
# from a previous run serves the OLD binary/state and silently poisons this
# one, and ros2 launch's own SIGTERM escalation can outlive a short window.
teardown() {
  pkill -f 'ros2 launch uav_bringup' 2>/dev/null
  pkill -f 'obstacle_hold_gate.py' 2>/dev/null
  sleep 3
...
echo
echo "=== 2/5 autonomy stack (sim.launch.py -- perception/safety/safety_enforcement all left at DEFAULT: false/true/true) ==="
BRINGUP_LOG=$HOME/gs3hold_bringup.log
...
echo
echo "=== 3/5 R27 dieu 1: confirm ZERO publisher on obstacle_map_local BEFORE the probe starts ==="
...
python3 -u "$PROBE" --uav-id "$UAV_ID" 2>&1 | tail -200
...
echo
echo "G-S3-HOLD exit status: $verdict"
exit "$verdict"
