...
For best runtime performance, keep only the subwindows needed for the current run visible, avoid enabling multiple high-resolution equirectangular image types on the same camera, and prefer the image APIs when exact depth, segmentation, infrared, annotation, disparity, or optical-flow data is required. A visible subwindow enables automatic updates for its capture type; hide it when the same type is being read at high rate by an image API, live ROS 2, CameraHost, or MCAP recording, because that explicit capture can otherwise render the target a second time.
