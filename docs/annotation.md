...
* Custom annotation layers support static and skeletal meshes. The built-in instance segmentation and infrared ID paths additionally support instanced static mesh components through generated annotation mirrors and Unreal Landscape components through a landscape annotation proxy.
  ...
  * Landscape support is currently limited to the built-in indexed instance segmentation and infrared ID paths. Custom RGB, greyscale, and texture annotation layers still gather `UMeshComponent` instances only.
  * For landscapes, each `ULandscapeComponent` is listed separately, while the owning `ALandscapeProxy` name is used as a shared label key for grouped ID updates.
  * UE foliage painted as instanced static mesh foliage is covered by the built-in instance segmentation and infrared ID paths because `UFoliageInstancedStaticMeshComponent` derives from `UInstancedStaticMeshComponent`. It still receives one ID/color per foliage component, not per individual tree/grass instance, and annotation materials do not reproduce wind or other source-material world-position-offset deformation.
  ...
  * Other unsupported primitive types, such as decals, text, non-instanced foliage systems, and non-mesh custom primitives, generally will not render in annotation captures because the captures render only generated annotation components in their show-only lists.
