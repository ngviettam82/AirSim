...
5. Every valid image is retained even when its timestamp delta exceeds `ImageSyncToleranceMs`. Default output saves it as a file with timing metadata; Rosbag output embeds it in the MCAP file with the frame-proven capture-state timestamp.
...
| `/airsim_node/<vehicle>/<camera>_<type>[_<annotation>]/image` | `sensor_msgs/msg/Image` | Raw RGB (`rgb8`) or float depth (`32FC1`) images, stamped with the frame-proven capture-state time. |
| `/airsim_node/<vehicle>/<camera>_<type>[_<annotation>]/image/compressed` | `sensor_msgs/msg/CompressedImage` | JPEG output for `Compress=true` or `ImageFormat="jpeg"`. The message `format` field is `jpeg`; the image is stamped with the frame-proven capture-state time. |
...
For every direct-bag image, the ROS `Header.stamp` and MCAP log time are the valid frame-proven capture-state timestamp (`ImageTimeStamp`): the game-thread simulation/camera state sampled immediately before explicit `CaptureScene()`, whose readback is ordered directly behind that capture. There is no fallback to the earlier physics latch, request time, readback completion, or writer time: if the render path cannot prove that transaction, the writer logs a warning and omits that image from the MCAP file. `image_metadata` preserves the earlier `FrameTimeStamp` physics/vehicle-render latch and the signed `ImageTimeStamp - FrameTimeStamp` delay, so fusion can apply a stricter state-association filter. This does **not** claim a frozen-world shutter or a hard camera-to-IMU time bound: at a nominal 333 Hz IMU rate, one sample period is about 3 ms, but fusion should use timestamps and interpolation rather than assume a fixed maximum displacement.

`ImageFormat="jpeg"` captures the same raw RGB render result and encodes it only on the recorder writer worker. JPEG encode time therefore cannot change the image's source timestamp or its associated capture transaction, but JPEG is lossy and should not be used for pixel-exact labels, segmentation IDs, or depth.
