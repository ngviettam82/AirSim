...
Equirectangular capture is configured in `settings.json` by setting `ProjectionMode` to `"Equirectangular"` for an existing `ImageType`. It is not requested as a separate API image type. The same `simGetImage` and `simGetImages` calls are used after the camera is configured.

For example, if `front_center` has `ImageType: 0` configured with `ProjectionMode: "Equirectangular"`, this request returns a scene equirectangular image:
