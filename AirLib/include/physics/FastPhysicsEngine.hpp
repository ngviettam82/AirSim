...
namespace airlib
{
    ...
    public:
            ...
            : enable_ground_lock_(enable_ground_lock), wind_(wind), ext_force_(ext_force)
        {
            ...
            // default turbulence off until configureWindTurbulence is called
            wind_turb_x_.initialize(2.0f, 0.0f, 0.0f);
            wind_turb_y_.initialize(2.0f, 0.0f, 0.0f);
            wind_turb_z_.initialize(2.0f, 0.0f, 0.0f);
        }

        void configureWindTurbulence(bool enabled, real_T sigma_mps = 1.5f, real_T tau_s = 2.0f)
        {
            wind_turbulence_enabled_ = enabled;
            wind_turb_x_.initialize(tau_s, enabled ? sigma_mps : 0.0f, 0.0f);
            wind_turb_y_.initialize(tau_s, enabled ? sigma_mps : 0.0f, 0.0f);
            wind_turb_z_.initialize(tau_s, enabled ? sigma_mps * 0.5f : 0.0f, 0.0f); // weaker vertical
            // GaussianMarkov allows re-reset without an intervening update.
            wind_turb_x_.reset();
            wind_turb_y_.reset();
            wind_turb_z_.reset();
            wind_turbulence_ = Vector3r::Zero();
        }
        ...
        virtual void update(float delta = 0) override
        {
            ...
            if (wind_turbulence_enabled_) {
                wind_turb_x_.update(delta);
                wind_turb_y_.update(delta);
                wind_turb_z_.update(delta);
                wind_turbulence_ = Vector3r(wind_turb_x_.getOutput(), wind_turb_y_.getOutput(), wind_turb_z_.getOutput());
            }
            else {
                wind_turbulence_ = Vector3r::Zero();
            }
    ...
    private:
        ...
        void updatePhysics(PhysicsBody& body)
        {
            ...
            const Vector3r effective_wind = wind_ + wind_turbulence_;
            getNextKinematicsNoCollision(dt, body, current, next, next_wrench, effective_wind, ext_force_);
        ...
        */
                                    ...
                                    const Vector3r& wind_world)
        {
            ...
            // Quadratic angular drag in body frame: τ = -c · ρ · ω |ω|
            // Uses PhysicsBody::getAngularDragCoefficient() (0 disables).
            const real_T ang_c = body.getAngularDragCoefficient();
            if (ang_c > 0) {
                const Vector3r omega = angular_vel_body;
                wrench.torque += -ang_c * air_density *
                                 Vector3r(omega.x() * std::abs(omega.x()),
                                          omega.y() * std::abs(omega.y()),
                                          omega.z() * std::abs(omega.z()));
            }
    ...
    private:
        ...
        Vector3r wind_;
        Vector3r ext_force_;
        bool wind_turbulence_enabled_ = false;
        Vector3r wind_turbulence_ = Vector3r::Zero();
        GaussianMarkov wind_turb_x_, wind_turb_y_, wind_turb_z_;
    };
}
} //namespace
#endif
